...
static void filltMipName(const cmft::Image& _image, uint8_t mip, uint32_t mipHeight, char *mipName, uint32_t mipWidth)
{
    ...
    bool imageSaveHdr(const char* _fileName, const Image& _image, bx::AllocatorI* _allocator)
    {
        ...
        for (uint8_t mip = 0, endMip = _image.m_numMips; mip < endMip; ++mip)
        {
            ...
            const uint32_t mipSize  = mipWidth * mipHeight * bytesPerPixel;
            
            filltMipName(_image, mip, mipHeight, mipName, mipWidth);
            ...
            write = fwrite(mipData, mipSize, 1, fp);
